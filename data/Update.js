...
async function uploadFsFiles() {
  ...
  let pathPrefix = document.getElementById('fs-path-input').value.trim();
  if (!pathPrefix.startsWith('/')) pathPrefix = '/' + pathPrefix;
  if (!pathPrefix.endsWith('/')) pathPrefix += '/';

  for (const file of selectedFsFiles) {
    const destName = pathPrefix + file.name;
    addLog(logList, 'spin', `Надсилання ${destName}...`);
    try {
      await uploadFileToFs(destName, file);
      updateLastLog(logList, 'ok', `${destName} — завантажено`);
    } catch (e) {
      updateLastLog(logList, 'err', `${destName} — помилка: ${e.message}`);
    }
  }
